...
### 4. Optional: server options & tuning

The binary is spawned by your MCP client, so CLI options go in the config `args` and environment variables in the `env` block.

| CLI option | Default | Effect |
|------------|---------|--------|
| `--port N` | `1994` | Port for the plugin bridge. If you change it, set the same port in the plugin's gear dialog — otherwise the bridge never connects |
| `--ip ADDR` | `127.0.0.1` | Bind address. `0.0.0.0` accepts remote connections; there is no authentication, so keep it on loopback unless you explicitly need remote access |

| Environment variable | Default | Effect |
|----------------------|---------|--------|
| `FIGMA_MCP_TIMEOUT` | `30` | Bridge timeout in seconds for all tools except `get_design_context` |
| `FIGMA_MCP_TIMEOUT_DESIGN_CONTEXT` | `60` | Bridge timeout in seconds for `get_design_context` |
| `FIGMA_MCP_FOLLOWER_TIMEOUT` | `65` | Follower → leader HTTP timeout in seconds (keep above the max bridge timeout) |
...
Example — custom port with a longer tool timeout:

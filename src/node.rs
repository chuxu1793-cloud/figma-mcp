...
impl Node {
    ...
    /// Holds the leader write-lock across port binding to prevent
    /// two concurrent callers from both becoming leader.
    pub async fn become_leader(&self) -> Result<(), String> {
        ...
        // Acquire leader write-lock BEFORE binding the port so that
        // a concurrent become_leader() call blocks until we finish.
        let mut leader_guard = self.leader.write().await;

        // Re-check role under the lock — another caller may have just become leader
        {
            let role = self.role.read().await;
            if *role == Role::Leader {
                return Ok(());
        ...
        *leader_guard = Some(leader);
        ...
        // Set role to Leader after the leader is installed
        {
